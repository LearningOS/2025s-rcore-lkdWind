# 功能总结
    
​	关于link，与linux的ln命令类似，对文件创建链接相当于在操作系统层面增加个INode，但是磁盘里共用统一片块，那相当于拥有相同的inode编号，我们先通过原文件名称获取编号，已知所有文件都在根目录。找到编号后就可以得到块位置id和offset，在创建inode时需要。接下来修改目录项，需要先扩展目录项的存储区域，后写入目录项。返回一个新的inode。

​	关于unlink，通过name找到根节点后计算目录项数目，循环读取根节点每个文件对比名称，找到index后，把末尾的目录项换到index处，减小目录大小。

​	stat就连两个项需要获取，ino就是blockid，nlink需要通过块位置来对目录里所有文件的块位置进行比较，相同计数加1.在返回时我和gettime的处理一样，将虚拟地址转换成物理地址后再转换成可变指针，将stat结构体指针放进去。
    
# 简答作业

ch6:
1. root inode是根目录，文件检索的索引，本身的内容就是关于每个文件名称和ino，仅仅是root inode损坏的化是不影响文件本身的，但是会损失文件定位，恢复会困难。文件系统也会挂载失败，

ch7:
1. Pipe 的实际应用示例：统计文件行数
    在 Linux 终端中，pipe（|）常用于将一个命令的输出作为另一个命令的输入。例如：
    bash
    cat file.txt | wc -l
    cat file.txt：输出文件内容到标准输出（stdout）。
    |：管道符，将 cat 的输出重定向到 wc 的输入。
    wc -l：统计输入的行数。
底层机制：操作系统会创建一个匿名管道，cat 的 stdout 连接到管道的写端，wc 的 stdin 连接到管道的读端。
2. 中央消息代理（Message Broker）
    核心思想：所有进程连接到一个中央代理（如 Unix Domain Socket 或共享内存），由代理负责消息路由。




# 荣誉准则

    1. 在完成本次实验的过程（含此前学习的过程）中，我曾分别与 以下各位 就（与本次实验相关的）以下方面做过交流，还在代码中对应的位置以注释形式记录了具体的交流对象及内容： 无
        2. 此外，我也参考了 以下资料 ，还在代码中对应的位置以注释形式记录了具体的参考来源及内容： 无
        3. 我独立完成了本次实验除以上方面之外的所有工作，包括代码与文档。 我清楚地知道，从以上方面获得的信息在一定程度上降低了实验难度，可能会影响起评分。
        4. 我从未使用过他人的代码，不管是原封不动地复制，还是经过了某些等价转换。 我未曾也不会向他人（含此后各届同学）复制或公开我的实验代码，我有义务妥善保管好它们。 我提交至本实验的评测系统的代码，均无意于破坏或妨碍任何计算机系统的正常运转。 我清楚地知道，以上情况均为本课程纪律所禁止，若违反，对应的实验成绩将按“-100”分计